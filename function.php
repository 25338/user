...
function getrec($conn, $table, $field, $where){
	$_sql="select ".$field." from ".$table." ".$where;
	$_res = $conn->query($_sql);
	return $_res->fetch_array();
}

function ShowTreeOption($conn, $ParentID, $lvl) { 
global $lvl; 
global $id_menu2;
global $pos_select;
$lvl++; 
...
<div class="modal fade" id="'.$Id.'" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      if($Url) {
        $s.='<a class="btn btn-primary" href="'.$Url.'" id="close"><i class="fas fa-check"></i> '.$kw["close"][1].'</a>';
      }
       $s.='
      </div>
    </div>
...
<div class="modal fade" id="'.$Id.'" tabindex="-1" role="dialog" aria-labelledby="ex_'.$id.'" aria-hidden="true">
  <div class="modal-dialog shadow" role="document">
    <div class="modal-content" style="background:#eee;">
      ...
      <div class="modal-body">'.$Text.'</div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal"><i class="fas fa-times"></i> '.$kw["no"][1].'</button>
        <a class="btn btn-primary" href="'.$Url.'" id="modal_but_yes"><i class="fas fa-check"></i> '.$kw["yes"][1].'</a>
      </div>
    </div>
  </div>
</div>
...
if( $page >= 2 ) {
	...
	$previous .= 'page=' . ( $page - 1) . '">'.$kw["back_page"][1].'</a>';
}

if( $page < $maxrec and ( $page * $max_view) < $maxrec ) {
	$next .= '<a class="page-link"  href="' . $url ;
	$next .= 'page=' . ( $page + 1) . '">'.$kw["next_page"][1].'</a> ';
}
  ...
  while( $cp++ < $tp ) {
	if( ( $cp < $page - $v_page or $cp > $page + $v_page) and $v_page != 0 ) {
		if( $cp == 1 ) {
			...
			$start .= 'page=1">'.$kw["start_page"][1].'</a>';
		}

		if( $cp == $tp ) {
			$end .= '<a class="page-link" href="' . $url;
			$end .= 'page=';
			$end .= $tp . '">'.$kw["end_page"][1].' </a>';
		}
	}else{
		if( $cp == $page ) {
...
function save_date($d, $v = 1){
	//20-02-2019
	$r=substr($d,6,4).substr($d,3,2).substr($d,0,2);

	if($v==2){
		//2019-02-20
		$r=substr($d,0,4).substr($d,5,2).substr($d,8,2);
	}
	return $r;
}

function load_date($d, $f, $v = 1){
	//$d=20180327 -> 27-03-2018
	$r=substr($d,6,2).$f.substr($d,4,2).$f.substr($d,0,4);

	if($v==2){
		//$d=20180327 -> 2018-03-27
		$r=substr($d,0,4).$f.substr($d,4,2).$f.substr($d,6,2);
	}
	return $r;
}

function load_datetime($d){
	//$d=20180327 -> 27-03-2018 12:23:33
	$f="-";
	$r=substr($d,6,2).$f.substr($d,4,2).$f.substr($d,0,4);
...
function getoption($conn, $table, $field, $where, $sel){
